...
echo "  Training files: $train_count (${TRAIN_RATIO}%)"
echo "  Validation files: $val_count ($((100 - TRAIN_RATIO * 100))%)"
...
echo "  Training indices: ${train_indices[*]}"
...
for idx in "${train_indices[@]}"; do
    ...
    echo "Downloading training file $((train_downloaded + 1))/$train_count: $filename"
    gsutil cp "$gcs_file" "$TRAIN_DIR/"
    train_downloaded=$((train_downloaded + 1))
    
    # Progress update every 10 files
    if [[ $((train_downloaded % 10)) -eq 0 ]]; then
        echo "Training progress: $train_downloaded/$train_count files downloaded"
    fi
done
...
for idx in "${val_indices[@]}"; do
    ...
    echo "Downloading validation file $((val_downloaded + 1))/$val_count: $filename"
    gsutil cp "$gcs_file" "$VAL_DIR/"
    val_downloaded=$((val_downloaded + 1))
    
    # Progress update every 10 files
    if [[ $((val_downloaded % 10)) -eq 0 ]]; then
        echo "Validation progress: $val_downloaded/$val_count files downloaded"
    fi
done
