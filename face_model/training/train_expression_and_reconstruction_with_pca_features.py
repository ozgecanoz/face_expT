...
Using PCA-projected DINOv2 features instead of raw face images

The model extracts expression tokens from PCA features and reconstructs full images (518x518 RGB).
Reconstruction loss compares the reconstructed images to the original frames stored in the H5 files.
"""
...
from data.dataset import FaceFeaturesDataset, create_face_features_dataloader
...
class JointExpressionReconstructionModelWithPCA(nn.Module):
    """
    Joint model combining Expression Transformer and Expression Reconstruction Model
    Uses pre-computed PCA-projected features instead of raw images
    """
    ...
    def forward(self, pca_features, subject_ids, clip_lengths=None):
        """
        Args:
            pca_features: (total_frames, 1369, 384) - PCA-projected DINOv2 features
        ...
        if clip_lengths is None:
            # Single clip
            return self._forward_single_clip(pca_features, subject_ids)
        ...
        for clip_length in clip_lengths:
            ...
            # Extract clip data
            clip_features = pca_features[start_idx:end_idx]  # (T, 1369, 384)
            ...
            clip_expression_tokens, clip_reconstructed_images = self._forward_single_clip(
                clip_features, clip_subject_ids
            )
    ...
    def _forward_single_clip(self, pca_features, subject_ids):
        """
        Forward pass for a single clip using PCA features
        """
        # Extract expression tokens using Expression Transformer
        expression_tokens = []
        adjusted_pos_embeddings = []
        
        for t in range(pca_features.shape[0]):
...
class ExpressionReconstructionLossWithPCA(nn.Module):
    """
    Combined loss for expression reconstruction using PCA features:
...
def prepare_pca_features_data(batch, device):
    """
    Prepare training data for expression reconstruction using PCA features
    
    Args:
        batch: Batch from dataloader containing 'features', 'frames', and 'subject_id' for each sample
        device: Device to use
    
    Returns:
        pca_features: (total_frames, 1369, 384) - All PCA features
        original_frames: (total_frames, 3, 518, 518) - All original frames for reconstruction loss
        subject_ids: (total_frames,) - Subject IDs for each frame
        clip_lengths: List of clip lengths
    """
    # Extract features and frames from all clips in the batch
    all_features = []
    all_frames = []
    subject_ids = []
    clip_lengths = []
    
    for i, (features, frames, subject_id) in enumerate(zip(batch['features'], batch['frames'], batch['subject_id'])):
        # features: (num_frames, 1369, 384) for one clip
    ...
    # Concatenate all features, frames, and subject IDs
    pca_features = torch.cat(all_features, dim=0).to(device)  # (total_frames, 1369, 384)
    original_frames = torch.cat(all_frames, dim=0).to(device)  # (total_frames, 3, 518, 518)
    subject_ids = torch.cat(subject_ids, dim=0).to(device)  # (total_frames,)
    
    return pca_features, original_frames, subject_ids, clip_lengths


def train_expression_and_reconstruction_with_pca_features(
    dataset_path,
    ...
    device="cpu",
    num_workers=0,
    pin_memory=True,  # Default to True for PCA features
    persistent_workers=True,  # Default to True for PCA features
    drop_last=True,
    ...
    feature_key='projected_features'
):
    """
    Train the joint expression and reconstruction model using PCA-projected features
    
    Args:
        dataset_path: Path to the PCA features dataset
        ...
        pin_memory: Whether to pin memory (default True for PCA features)
        persistent_workers: Whether to use persistent workers (default True for PCA features)
    ...
    """
    logger.info(f"Starting joint expression and reconstruction training with PCA features on device: {device}")
    ...
    if device.type == "cuda":
        ...
        logger.info("Applied CPU memory optimizations")
    
    # Create checkpoint directory
    os.makedirs(checkpoint_dir, exist_ok=True)
    
    # Load dataset using PCA features
    dataset = FaceFeaturesDataset(dataset_path, max_samples=max_samples, feature_key=feature_key)
    dataloader = create_face_features_dataloader(
        dataset_path, 
    ...
    if val_dataset_path is not None:
        val_dataset = FaceFeaturesDataset(val_dataset_path, max_samples=max_val_samples, feature_key=feature_key)
        val_dataloader = create_face_features_dataloader(
            val_dataset_path, 
            batch_size=batch_size, 
    ...
    if joint_checkpoint_path is not None:
        ...
        try:
            ...
            joint_model = JointExpressionReconstructionModelWithPCA(
                expr_embed_dim=expr_embed_dim,
                expr_num_heads=expr_num_heads,
                expr_num_layers=expr_num_layers,
                expr_dropout=expr_dropout,
    ...
    criterion = ExpressionReconstructionLossWithPCA(
    ...
    if log_dir is None:
        log_dir = f"./logs/exp_recon_pca_training_{job_id}_{timestamp}"
    else:
        log_dir = os.path.join(log_dir, f"exp_recon_pca_training_{job_id}_{timestamp}")
    
    os.makedirs(log_dir, exist_ok=True)
    writer = SummaryWriter(log_dir)
    
    logger.info(f"📊 TensorBoard logging to: {log_dir}")
    logger.info(f"🆔 Job ID: exp_recon_pca_training_{job_id}")
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(progress_bar):
            # Prepare data using PCA features and original frames
            pca_features, original_frames, subject_ids, clip_lengths = prepare_pca_features_data(batch, device)
        
            # Forward pass through joint model
            all_expression_tokens, all_reconstructed_images = joint_model(pca_features, subject_ids, clip_lengths)
            ...
            for clip_idx, (clip_expression_tokens, clip_reconstructed_images) in enumerate(zip(all_expression_tokens, all_reconstructed_images)):
                ...
                clip_original_images = original_frames[start_idx:end_idx]  # (T, 3, 518, 518)
            ...
            if current_training_step % 50 == 0 and device.type == "cuda":
                ...
                import gc
                gc.collect()
            elif current_training_step % 50 == 0 and device.type == "cpu":
                # CPU memory cleanup
                import gc
                gc.collect()
            ...
            if current_training_step % save_every_step == 0 and expression_tokens_by_clip:
                ...
                # Joint model
                joint_step_path = os.path.join(checkpoint_dir, f"joint_expression_reconstruction_pca_step_{current_training_step}.pt")
                save_checkpoint(
                ...
                # Individual models
                expr_step_path = os.path.join(checkpoint_dir, f"expression_transformer_pca_step_{current_training_step}.pt")
                save_checkpoint(
                ...
                recon_step_path = os.path.join(checkpoint_dir, f"expression_reconstruction_pca_step_{current_training_step}.pt")
                save_checkpoint(
        ...
        if device.type == "cuda":
            ...
            import gc
            gc.collect()
            logger.info(f"🧹 GPU memory cleanup after epoch {epoch+1}")
        else:
            import gc
            gc.collect()
            logger.info(f"🧹 CPU memory cleanup after epoch {epoch+1}")
        ...
        if val_dataloader is not None:
            val_loss = validate_expression_reconstruction_with_pca(
                joint_model, val_dataloader, criterion, device
            )
...
def validate_expression_reconstruction_with_pca(
    joint_model,
    val_dataloader,
    criterion,
    device="cpu"
    ...
    """
    Validate the joint expression reconstruction model using PCA features
    """
    joint_model.eval()
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(val_dataloader):
            ...
            for clip_idx, (clip_expression_tokens, clip_reconstructed_images) in enumerate(zip(all_expression_tokens, all_reconstructed_images)):
                ...
                # Extract original images for this clip
                clip_original_images = original_frames[start_idx:end_idx]
...
def test_joint_expression_reconstruction_with_pca():
    """Test the joint expression reconstruction model with PCA features"""
    print("🧪 Testing Joint Expression Reconstruction Model with PCA Features...")
    
    # Create model
    model = JointExpressionReconstructionModelWithPCA()
    print("✅ Joint model with PCA features created successfully")
    ...
    # Create dummy PCA features data
    pca_features = torch.randn(batch_size * num_frames, 1369, 384)
    subject_ids = torch.randint(0, 100, (batch_size * num_frames,))
    clip_lengths = [num_frames, num_frames]
    
    # Forward pass
    expression_tokens, reconstructed_images = model(pca_features, subject_ids, clip_lengths)
    ...
    print("✅ Joint expression reconstruction with PCA features test passed!")


if __name__ == "__main__":
    test_joint_expression_reconstruction_with_pca() 